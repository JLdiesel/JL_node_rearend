...
class MomentService {
  ...
  async getMomentByUserId(id) {
    const statement =
      'SELECT user_id userId,JSON_ARRAYAGG(content) contentArr FROM moment GROUP BY user_id HAVING user_id=?';
  ...
  async getMomentList(offset, size) {
    const statement = `SELECT m.id id,m.content content,m.createAt createTime,m.updateAt updateTime,
            ...
            (SELECT COUNT(*) FROM moment_tag mt WHERE mt.moment_id=m.id) tagCount
             (SELECT JSON_ARRAYAGG(CONCAT('http://localhost:3000/moment/images/',file.filename)) FROM file WHERE m.id=file.moment_id) images 
            FROM moment m
