...
class VideoService {
  async updateAvatarUrlById(avatarUrl, videoId) {try{
  ...
  async createVideo(title, status,ezcontent) {try{
  ...
  async getVideoListBystatus(status, offset, top) {try{
...
LIMIT ?,? ;`;
  ...
  //判断是否含有tag
  async hasTag(videoId, tagId) {try{
  ...
  //为文章添加标签
  async addTag(videoId, tagId) {try{
  ...
  async getVideoInnerById(videoId) {try{
...
WHERE vf.video_id=?`;
  ...
  async getVideoCommentById(videoId) {try{
